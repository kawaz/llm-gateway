...
DR-0030 §3 (自主レート制限) と §4 (ns の allowlist) を実装に落とす計画。永続化の契約は DR-0031 §2 (3) `CounterStore`。本書は推しを書くが裁定はしない (末尾「裁定が要る点」)。
...
**推し: A を先に作り、B を後続の段として設計だけ揃える。** カウンタと判定の部品は credential の種類を知らない形 (鍵は `CredentialId`) で core に置くので、B は「LLM 経路の credential 確定点で同じ部品を呼ぶ」だけで足せる。
...
- 現状 `[secrets]` は置き場の設定 (`type = "file"`) を持つ表なので、`[secrets.<id>]` と同居させると `type` と id の名前空間が衝突する。案: (a) 置き場を `[secrets.store]` / `[store]` へ移す、(b) 宣言を `[secret.<id>]` (単数) で分ける、(c) `[upstreams.<name>].limits` に書く。**(c) は単位が行き先になり、同じ secret を 2 つの upstream で共有すると枠が割れるので DR-0030 §3 に反する。推しは (a)** (DR-0031 §4 の `store = "file"` とも揃う)
...
- 照合は `pattern::matches` の glob (DR-0030 §4 の「パス prefix」は `/v1/*` で表す)。メソッドに `*` を許すかは裁定点 (推し: 許さない。副作用のある method を明示させるのが §4 の目的)
- `allow` を書かない ns の扱い: **推しは「何も通さない」(閉じた既定)**。ただし既存の LLM 経路 (`/ns-<ns>/v1/...`) の挙動は変えない (DR-0030 Consequences) ので、第一版の allowlist はパススルーにだけ効かせ、LLM 経路は `llm` キーを段 4 で導入するまで対象外。これは既存 ns が段 3 の本番投入で突然パススルーを使えなくなる変化を伴う (段 3 のパススルーは現状 ns 側の制限なしで通る) ので、切替の告知が要る
...
毎 request の書き込みは `keep_saving` の「1 リクエストごとに書くのは無駄」(kawaz 裁定) と衝突する。ただしあれは閲覧用 stats の裁定で、fail-closed の枠には「書き損ねた分だけ枠を超える」(DR-0031) ので周期書きは契約違反になる。緩める案として **予約 (lease) 方式**: 一度に `k` 件分を先に書いて (`write_own(累計 + k)`)、メモリで消化し、使い切ったら次を書く。他 writer から見ると最大 `k × writer 数` だけ早く満杯に見える (安全側)。`k` は `limit` の数 % などで決める。第一版は `k = 1` (毎回書く) を推す — 日次の上限を持つ API は流量が低いのが普通で、測ってから緩める。

### 3.3 `missing` のときの status

DR-0031 は「通さない」まで。候補:

- **503 + `Retry-After`** (推し): 枠が埋まったのではなく gateway が判定できない。429 にするとクライアントは「枠切れ」と読み、`X-RateLimit-*` とも矛盾する
...
- tz の実装に crate が要る: 推しは `jiff` (IANA DB を同梱でき、DST の曖昧時刻の規則が明示的)。依存追加は裁定点

### 3.5 `Retry-After` と `X-RateLimit-*`

- 超過したバケットのうち **窓の終わりが最も遅いもの** を `Retry-After` (秒、切り上げ) にする。早いものを返すとクライアントは戻ってきてまた断られる
- `X-RateLimit-Limit` / `-Remaining` / `-Reset`: 複数バケットのうち **残量の比率が最も小さい (最も詰まっている) 1 つ** を出す。`-Reset` は形の揺れ (epoch 秒 / 残り秒) がある。推しは **残り秒** (IETF draft の `RateLimit-Reset` と GitHub 以外の多くの API に合わせる) だが裁定点
- 通した応答にも付ける。パススルーは「応答を触らない」(DR-0030 §2) が、これは §3 が明示的に足すヘッダ。上流が同名ヘッダを返した場合は **上流のものを落として gateway のものを載せる** か、gateway 側を `X-Gateway-RateLimit-*` にするかが裁定点 (DR-0030 §3 は「上流の値を混ぜない」なので同名の併存は不可。推しは上流の同名を落とす。便乗で読みたいクライアントは無いと見る)
...
- **同時到着の競合**: 同一 writer 内は mutex で直列化できるが、writer 間は「読む → 判定 → 書く」の間に相手が書く。両者が残り 1 を見て両方通す。上限超過は writer 数 − 1 件まで。厳密にするには bucket ごとの flock (DR-0031 (1) 相当) が要り、合算カウンタの意味論を外れる。第一版は許容と明記する案
...
- **ns allowlist の閉じた既定** は段 3 のパススルーを使っている既存 ns を切替時に止める
- 上流の `x-rate-limit-*` 便乗 (DR-0030 §3 末尾) は本計画に入れていない。必須でないので段 3 の範囲外とする

## 裁定が要る点

1. 適用範囲: パススルーだけで始めるか (推し)、LLM 経路にも段 3 内で掛けるか
2. `[secrets]` の置き場設定と `[secrets.<id>]` 宣言の衝突の解き方 (推し: 置き場を `[secrets.store]` 等へ移す)
3. ns に `allow` を書かない時の既定 (推し: 閉じる) と、既存 ns の切替手順
4. ns allowlist の method `*` を許すか (推し: 許さない)
5. `missing` 時の status (推し: 503)
6. `X-RateLimit-Reset` の形 (推し: 残り秒) と、上流の同名ヘッダの扱い (推し: 落とす)
7. 日次の書き込み頻度 (推し: 第一版は毎 request で書く、予約方式は後で)
8. writer 間の同時到着で上限を僅かに超えることの許容
9. tz crate (`jiff`) の依存追加
