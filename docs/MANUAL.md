...
- Each gateway's file is named after its listen address, and two gateways cannot listen on the same address, so a file has only one writer and needs no lock
- A window's count is removed from the file once the window ended more than one window ago. The current, the previous and any later window are kept, so a clock that steps back to the previous day or month finds its count still there
