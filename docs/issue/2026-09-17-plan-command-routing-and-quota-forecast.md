...
its week %、各枠の経過% と使用%の対)。JSON は同じ構造。
...
2026-09-17 追記 (kawaz): 消費者は統括 (CLI、pull) だけでなく ccmsg webui のステータス
バー (人の判断材料) も想定。ペース判定は履歴不要の形でスコープに含める: 枠の窓 (5h /
週) の経過 % と使用 % の対を出力に入れる (経過 40% で使用 70% = 速い、経過 80% で使用
30% = 余り)。「控えて / 贅沢に」の文言化は読む側の責務のまま。gateway の口ができたら
ccmsg リポへ「plan を polling して表示」の依頼 issue を出す。
...
2. ペース判定は履歴不要の形でスコープに含める — 枠の窓 (5h / 週) の経過 % と使用 % の
   対を出力に入れる (例: 経過 40% で使用 70% = 速い、経過 80% で使用 30% = 余り)。
   助言文 (「控えて」「贅沢に使え」等の文言化) は入れない — 方針の適用は読む側
   (統括 / ccmsg webui) の責務
...
- [ ] 助言文 (「控えて」「贅沢に使え」等の文言化) は含まれない (設計方針 2 の逸脱が
      ないことを確認)
