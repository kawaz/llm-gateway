...
設定のパス欄 (`extends`、`[server] binary_path`、`[store] dir`、`[stats] dir`、`[webhook] token_file`) は、読み込むときに 1 回だけ `~` と環境変数を開く。

| 書き方 | 意味 |
| --- | --- |
| 先頭の `~` / `~/` | `$HOME`。途中の `~` はただの文字 |
| `$VAR` | その環境変数 |
| `${VAR}` | 同上。後ろに文字が続くときに使う |
| `${VAR:-代わり}` | 定まっていなければ「代わり」。こちらも同じ規則で開く (`${XDG_STATE_HOME:-~/.local/state}`) |

空の環境変数は「定まっていない」と数える。代わりを書いていない `$VAR` が定まっていなければ読み込みを止め、変数名を名指しで言う — 空として通すと、`~/.local` のつもりの値が根を指したまま動いてしまう。`\$` の逃がし方は無い。

省略したときの既定値も同じ規則で組み立てる (`$XDG_STATE_HOME` が無ければ `~/.local/state`) ので、既定の場所を設定に書き写すなら `${XDG_STATE_HOME:-~/.local/state}/llm-gateway/stats` と書けばよい。開いた後の実パスは `llm-gateway check --config <path>` が出力し、存在の確認もその値に対して行う。
...
```toml
[stats]
uncached_keep = 50   # 新しい順にこの件数まで (既定 50)
...
| 本文を経路の形式へ変換できない (例: OpenAI 経路で `tool_result` に `document` ブロックがある)、`model` が無い | 400 | `invalid_request_error` |
| 未知の namespace | 404 | `invalid_request_error` |
| モデルがどの経路にも無い | 404 | `not_found_error` |
| 全経路が失敗 / upstream に届かない | 503 | `api_error` |
| upstream は応えたが応答が使えない (ストリームの形が想定外、大きすぎる、読み取りが途中で切れた) | 502 | `api_error` |
| gateway 自身かその設定の問題 | 500 | `api_error` |
...
- 各 gateway のファイル名は待ち受け先で、同じ待ち受け先の gateway は 2 つ同時に立たないので、1 つのファイルの書き手は常に 1 つで排他は要らない
- 窓の数は、その窓が終わってから 1 窓分以上たったところでファイルから消す。今の窓・直前の窓・先の窓は残すので、時計が前の日 / 月へ戻っても、その窓の数は残っている
