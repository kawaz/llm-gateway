...
| パス | 意味 | 認証 | 内部ルーティング |
|---|---|---|---|
| `/healthz` | プロセスが応答できるか | 不要 | **されない** (トップ専用の口) |
| `/ns-<name>/healthz` | その namespace が使える状態か | namespace の設定に従う | — |

トップの `/healthz` だけが内部ルーティングの例外になる。「トップのヘルスチェック
とか以外は自動で /ns-default 内部ルーティング」がこれにあたる。プロセスの生死は
namespace の設定と無関係に答えられる必要があるため。
