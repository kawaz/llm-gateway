...
`/healthz` はトップレベルにだけ置く。内部ルーティングの唯一の例外で、
`/ns-default/healthz` へは書き換えない。プロセスの生死は namespace の設定と
無関係に答えられる必要があるため。
...
この非対称性が重要で、**衝突したときに動かすのは常にこちら側**になる。upstream の
API 名は向こうが決めるので譲れない。将来 upstream が `/healthz` を使い始めたら、
gateway の死活監視は別の名前へ移す。namespace の下に自前のパスを持たないのは、
そのときに動かす対象をトップレベルだけに閉じ込めておくためでもある。
