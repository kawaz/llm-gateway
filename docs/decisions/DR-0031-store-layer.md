...
| usage スナップショット | `persist.rs` + `quota.rs` | `observed_at` の新しい方が勝つ (LWW)。一時ファイル経由の原子的な置き換え |
...
trait Persistence {
    type Value;
    type Guard;               // drop で手放す
    fn lock(&self, id) -> Result<Self::Guard>;   // 取れるまで待つ
    fn load(&self, id) -> Result<Self::Value>;
    fn store(&self, id, value: &Self::Value) -> Result<()>;
    fn version(&self, id) -> Result<Version>;    // 値が違えば他者が書いた
    fn list(&self) -> Result<Vec<Id>>;
}
```

- `lock` / `load` / `store`: **fail-closed**。refresh は失敗したら古い token を使い続けるのでなく失敗を返す (二重 refresh で refresh token を焼く事故を防ぐ)。静的 secret と JWKS の読み出しも fail-closed で、読めなければ認証を通さない
- `version`: **fail-closed**。版が取れない時に「変わっていない」とみなすと、他者の更新を取りこぼす
- 値の型は関連型 (または型パラメータ) にして、OAuth credential と静的 secret と JWKS が同じ backend に乗るようにする。静的 secret と JWKS は refresh しないので、読み手は `load` + `version` だけを使う (DR-0022 の「版が変わったら読み直す」は同じく効く)
...
- **fencing token**: 担当が替わるたびに単調に上がる番号を `Lease` に持たせる。担当が外部に副作用を起こす時 (keepalive の送信、控えの書き戻し) はその token を添え、backend は自分が知る最新より古い token の書き込みを拒む。期限切れに気づかず動き続けた元担当が送る・書く事故を、期限の判定でなく token の比較で止める
- 期限の判定に使う時刻は backend の時刻 (file では OS の flock が期限を持たないので下の §3 を参照)
...
- `write_own`: **best-effort**。書けなければ次の flush で書き直す。失敗で本流 (転送) を止めない
- `read_merged`: 閲覧用 (stats) は **best-effort** で、読めない writer の分は欠けたまま返し欠けたことを示す。レート制限の判定に使う日次バケットは、欠けると枠を超えて通してしまうので **fail-closed** (読めなければ判定できないとして止める)。どちらに倒すかは trait でなく呼び出し側の用途で決まるので、契約には「欠けを呼び出し元に伝える」までを書く
...
trait SnapshotStore<T> {
    fn put(&self, key, value: &T, observed_at) -> Result<()>;  // 古ければ捨てる
...
- 同じ `observed_at` の衝突は writer 名で決着させる (どちらが勝ってもよいが、全ノードで同じ方が勝つこと)
...
| (2) `LeaseStore` | keepalive の `.lock` を非ブロッキング flock で掴む。期限はプロセスの寿命 (flock はプロセスが死ねば外れる) | 単一ホストの flock は「期限切れの元担当」が生じない (外れた時点でその担当のプロセスは居ない) ので token は検査しない。値だけは形として持つ (下の未確定) |
| (3) `CounterStore` | `stats.rs` の writer 別ファイル + 閲覧時の `merge` | — |
| (4) `SnapshotStore` | `persist.rs` の原子的置き換え + `observed_at` の比較 | 時刻源は `observed_at` |
...
- 案 C: 別プロセスの store (gateway の外に状態サーバを立てる)
  - 不採用理由: 単一ホストの今の運用に常駐プロセスを 1 つ増やす。cache-warden が本来目的で使えるようになれば、それが別プロセスの store として backend に入るので、自前で作る理由が無い
...
- **fencing token は file backend では検査されない**: 単一ホストの flock では不要なので、token を持つ形だけ先に入れ、検査は複数ホストの backend で効く。file のテストで token の検査を固定できないので、2 つ目の backend を入れる時に契約のテストを足す必要がある
...
- fencing token の永続化形式 (file backend で値をどこに持つか — `.lock` ファイルの中身か、控えの JSON のフィールドか。検査しない間も値を上げておくかを含む)
...
- [DR-0022](DR-0022-credential-update-triggers-discovery.md) — credential の版の変化を契機に動く
