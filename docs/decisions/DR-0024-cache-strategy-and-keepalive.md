...
本 DR は「どの本文にどの cache 戦略を効かせるか」と「keepalive をいつ仕掛けて
いつ止めるか」を決める。止まった cache を実際に繋ぐ手段 (gateway による自送信) は
DR-0027。
...
- したがって **「実リクエストは 5m、cache を繋ぐ 1 本だけ 1h」は成立しない**。
  その 1 本が毎回「前回の 1h 境界から先」を 2 倍で書き直すので、常時 1h より
  高くつく (実測 2026-09-03: プレフィックス一致・同一 route・5 分以内で
...
`keepalive` は **`1h` + idle を検知した refresh**。本文の扱いは `1h` と同じで
(実リクエストも refresh も全ブレークポイントが `ttl: "1h"`)、違うのは会話が
止まったときに 1 往復を誘発して cache を次の 1 時間へ繋ぐかどうかだけ。繋ぐ
手段は DR-0027 の自送信で、本節が決めるのは**いつ仕掛け、いつ見送り、いつ畳むか**。

1. **debounce**: main の実リクエストを転送するたびに、会話系列 (DR-0012 の
   `prefix` + session_id) ごとのタイマーを **送出時刻 + 55 分** で再武装する。
   会話が動いている間は次のリクエストのたびに先送りされるので発火しない
2. **横断条件 — 通った先**: 系列は**直前の実リクエストが通った先** (namespace /
   モデル / route) も覚える。発火した時点でその route が締め出されている /
   候補から外れているなら**送らない**。送っても延ばしたい cache は別の
   credential の側にあり、そこにプレフィックスは無い。塞がりは解けるものなので
   見張りは畳まず、+55 分で次を試す (`keepalive_horizon` は据え置き)
3. **横断条件 — 対象**: origin が sub でない、`tools` を持つ (= 会話の本流)
   リクエストだけを見張る
4. **見張る期間**: 再武装は実リクエストでも refresh でも +55 分。ただし
   **見張る期間と通った先を延ばせるのは実リクエストだけ**で、最後の実リクエスト
   から `keepalive_horizon` を超えた系列は畳む
5. 知らせ (DR-0012 の request event) に**連鎖の姿** (`cache_since` /
   ...
   の 1 時間で終わりを描くが、繋いでいる系列は実際にはその先まで生きる。
   何番目かを数えるため、見張りは**出した本数と連鎖の起点**も持つ

#### 期限は壁時計で見る

送る直前に、繋ぐ相手 (cache の期限と見張る期間) が壁時計でまだ在るかを確かめ、
過ぎていれば送らずに見張りを畳む。次の実リクエストが張り直す。
...
見張る期間もとうに終わっている。そこへ出す 1 本は延ばす cache を持たず、
**プレフィックス全量の 1h write** (2 倍単価) をさせるだけになる — read 1 回で
済ませるはずが、最も高い形の書き直しを誘発する (実測 2026-09-10: サスペンド
復帰直後に 4 本)。
...
しばらく触らないと分かっている会話には、繋ぐ価値がない。繋がるのは使われない
cache だけで、1 時間ごとに 1 往復ぶんの read を払い続ける。止める口を持つ:

1. `POST /llm-gateway/keepalive/pause` に `{"session_id": "<id>"}`。認証は
   usage / status と同じ扱い (掛けない、手前の tailnet の境界を信頼する)。
   できるのは送り直しを止めることだけで、転送も認証情報も動かない
2. 受けた側は、その会話の id を持つ**全系列**の見張りを畳む。止めたいのは
   会話であって系列ではないので、頼む側がどの `prefix` で走っているかを
   知っている必要はない
3. 止めた状態は持たない。**見張りを畳むことが停止**で、次にその会話から実
   リクエストが来れば普通に張り直る (= 人が戻ってきた)。人へ向けた解除 API は
   持たない
...
Opus 5.5 は 0.05 倍なので 40 回ぶん = 36.7 時間、Opus 5 は 0.1 倍なので
20 回ぶん = 18.3 時間。同じ `0.3` がそれぞれ 22 時間、11 時間、5.5 時間になる。過去 7 日の実測では **0.2〜0.35** が目安
...
- **実リクエストは 5m のまま、cache を繋ぐ 1 本だけ 1h を付ける**: 1h フラグは
  1h エントリしか読まないので、その 1 本が毎回「前回の 1h 境界から先」を 2 倍で
  書き直す (実測: read 46K / 1h write 664K)。常時 1h より高い
- **5m で繋ぐ**: 5m エントリに `ttl: "1h"` を付けてヒットさせても TTL は昇格せず、
  5 分を過ぎた後の 1 本は全量 2 倍書きになる (実測)。繋ぐ側を 1h に揃える方が安い
...
- DR-0027 (cache を繋ぐ手段 = 自送信)、DR-0012 (request event / prefix)、
  DR-0014 (preset 境界)、DR-0016 (本文に触る際の教訓)
