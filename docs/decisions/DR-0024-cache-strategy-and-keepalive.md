...
3. 発火したら nonce (32 バイトを base64url にした 43 文字。中身は乱数と
   連鎖の起点・終わりで、割り方は後述の「合図の終わりは合言葉が持ち歩く」) を発行し、
...
| 受け取り済みの自分の合言葉 (`spent`) | 何もしない (transcript に残っているだけ) |
...
4. **解くのは、その会話から来た合言葉を持たない実リクエスト 1 本だけ**。
   人へ向けた解除 API は持たない。合図の往復 (`applied` / `late` / `foreign`)
   は解除に数えない — 止めた会話に最後の合図が出たままだったとき、その戻りで
   自分を解いてしまっては止めた意味がなくなる。解いた instance は兄弟へも
   それを渡す (下記)
...
- 解除も同じ経路で渡す。実リクエストは手前の振り分けが選んだ片方にしか来ない
  ので、伝えないともう片方が止まったまま残り、こちらが落ちたときに見張りを
  引き継げない。渡す先は内部用の `POST /llm-gateway/keepalive/resume`
  (`{"session_id": "<id>"}`、印と送りっぱなしの扱いは停止と同じ)。**実際に
  止まっていたのを解いたときだけ**送る — 大半の実リクエストは止まっていない
  会話のもので、毎回送れば往復が常時 1 本増える

転送も経路選定も、これまでどおり互いの存在を知らないまま進む。一覧に書いた
相手へ渡るのは keepalive の停止と解除だけ。

#### 合図の終わりは合言葉が持ち歩く

上表の収束は「誰が次を出すか」だけを決めていて、**いつ止めるか** (`keepalive_horizon`)
は各プロセスが自分で数えていた。控えに回る側がその系列を見たことがないと
(= 実リクエストを受けていない、あるいは自分の期間が尽きて畳んだ後)、期間の
起点を「相手の合図が見えた今」に取り直す。2 プロセスが交互に相手の合図を
`foreign` として受けると、そのたびに終わりが作り直されて合図が止まらない
(実測: 5.5 時間の期間を持つ会話へ 19 本 ≒ 16.5 時間の合図)。

期間は**系列に 1 つ**で、延ばせるのは実リクエストだけ。合図を見た側がそれを
知るために、**合言葉自身に連鎖の起点と終わりを持たせる**:

- 合言葉は 32 バイトを base64url にした 43 文字のまま。中身を
  **16 バイトの乱数 ‖ 起点 (Unix ms, i64 BE) ‖ 終わり (Unix ms, i64 BE)** に割る。
  文字数も頭 (`LLMGW-KEEPALIVE-`) も変わらないので、受け口の側 (ccmsg) と
  合言葉を返す会話には何も影響しない
- `foreign` を受けた側は、この系列を見ていなければ合言葉に書いてある起点と
  終わりで控えを作る。**終わりを過ぎていれば控えを作らない** — 期間の尽きた
  会話に引き継ぐものは無い
- 時刻を読めない合言葉 (= 別の作りで出されたもの) でも控えを作らない。
  32 バイトの乱数はどれも 43 文字として読めてしまうので、「時刻として通る値か」
  で見分ける (両欄が 2020〜2100 に収まる確率は 1e-14)。両方の作りが並ぶのは
  入れ替えの間だけで、そこで控えを 1 周期取りこぼしても実リクエストが張り直す
- 自分が出した合言葉かどうか (`applied` / `late` の判定) は、これまでどおり
  出したまま戻っていない合言葉の表で見る。乱数は 16 バイト = 128 ビット残る
  ので、推測して 1 時間を付けさせることはできない

合言葉は返信の後も会話に残るので、同じ会話の後続のリクエスト (分類器・要約
など、`system` の先頭が違うので**別系列**) がそのまま運んでくる。表から消す
だけでは、これを `foreign` = 別のプロセスの合図と読み違え、その系列に控えが
立つ。そこで**受け取り終えた合言葉は自分のものとして覚え**、運ばれてきた分は
`spent` として受け流す (見張りはその合図の分を既に織り込んである)。覚えるのは
合言葉自身が持つ連鎖の終わりまで — そこを過ぎれば控えの材料にもならない。

控えは 57 分後に出るので、**そこまでに相手の期間が尽きるなら
控えを作らない**。引き継いだ瞬間に出すだけで、引き継ぐ相手も既に黙っている。

#### 期限は壁時計で見る

合図を出す直前に、繋ぐ相手 (cache の期限と見張る期間) が壁時計でまだ在るかを
確かめ、過ぎていれば合図を出さずに見張りを畳む。次の実リクエストが張り直す。

予定を運ぶ単調時計 (`Instant` / tokio のタイマー) は**機械が眠っている間
止まる** (macOS のサスペンド、Rust の `Instant` は uptime 由来)。復帰した瞬間
は「予定の時刻はまだ先」「期限は先」と見えるが、壁時計では 1 時間の cache も
見張る期間もとうに終わっている。そこへ出す合図は延ばす cache を持たず、
受け取った会話に**プレフィックス全量の 1h write** (2 倍単価) をさせるだけに
なる — ping 1 回の read で済ませるはずが、最も高い形の書き直しを誘発する
(実測 2026-09-10: サスペンド復帰直後に 4 本)。

そのため見張りは期限と期間を **Unix ミリ秒 (壁時計) で持ち、それを正とする**。
単調時計の側は予定を起こすためだけに残す (タイマーの時計は変えない — 眠って
いた分の遅れは、起きた後の判定で捨てれば足りる)。置き場へ書く時刻も壁時計の
値をそのまま落とす (単調時計から起こし直すと、眠っていた分だけ先へずれる)。

不採用:

- **兄弟 (peers) へ見張りの状態を配る**: 停止と同じ経路で配れるが、台数分の
  往復と状態の同期が要る。合言葉は必ず両者を通るので、配らなくても同じものが
  届く
- **`foreign` では控えを作らない**: 連鎖はたしかに止まるが、出していた側が
  落ちたときに誰も引き継がない。多プロセス運用の狙い (観測だけで 1 本へ収束)
  を失う
