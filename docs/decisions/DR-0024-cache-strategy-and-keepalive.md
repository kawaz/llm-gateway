...
合言葉は返信の後も会話の transcript に残るので、**受け取り終えた合言葉は
覚えておく** (2026-09-09)。表から消すだけにしていた間は、同じ会話の後から来る
1 本 (分類器・要約など、`system` の先頭が違うので別系列) が transcript ごと
運んでくるたびに `foreign` = 別のプロセスの合図と読み違え、その系列に控えを
立てていた。受け取り済みと分かる合言葉は `spent` として何もしない (見張りは
その合図の分を既に織り込んである)。覚えるのは合言葉自身が持つ連鎖の終わりまで
で、そこを過ぎれば控えの材料にもならない。あわせて、**控えが出る 57 分後まで
に相手の期間が尽きるなら控えを作らない** — 引き継いだ瞬間に出すだけで、
引き継ぐ相手も既に黙っている。
