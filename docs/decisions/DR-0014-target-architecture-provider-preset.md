...
- **採用後は逐次 flush する**。採用前判定で読んだ先頭を含め、upstream が送った分は次の chunk を待たずにクライアントへ流れる。`crates/llm-gateway/src/gateway.rs` のテスト `a_messages_stream_is_flushed_chunk_by_chunk` / `a_responses_stream_is_flushed_chunk_by_chunk` / `a_kept_main_stream_is_flushed_chunk_by_chunk` で固定
