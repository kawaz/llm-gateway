...
//! # なぜ preset の側にあるか
//!
//! 単価はモデルを出している upstream が決める事実で、集計の器 ([`crate::stats`])
//! の関心ではない。core が規定するのは「読み出しのたびに換算する」形だけで、
//! どのモデルにいくら掛かるかは答えた provider が知っている (DR-0014 §4)。
//!
//! 表を preset ごとに割らず 1 枚にしてあるのは、**行の鍵がモデル名**だから。
//! 素通しの経路 ([`super::relay`]) は転送先が扱うモデルを何でも運ぶので、
//! 束ね方ごとに表を分けると同じ行を写すことになる。
//!
//! # なぜソースに埋め込むか
//!
